// File: Posttest/PosttestAnswerText.js

const express = require('express');
const router = express.Router();

module.exports = (supabase, authenticateToken) => {
  // GET: ดึงคำถามและคำตอบสำหรับ posttest
  router.get('/posttest-answertexts', async (req, res) => {
    const { instrument_id, posttest_id, user_id } = req.query;

    if (!instrument_id || !posttest_id) {
      return res.status(400).json({ 
        status: 'error', 
        message: 'กรุณาระบุ instrument_id และ posttest_id' 
      });
    }
    ...
    try {
      ...
      // กรองเฉพาะคำถามที่มีคำตอบหรือมี matching
      ...
      // ถ้ามี user_id ให้กรองคำถามที่ยังไม่ถูกตอบโดยผู้ใช้คนนั้น
      if (user_id) {
        const parsedUserId = parseInt(user_id);
        if (!isNaN(parsedUserId)) {
          // ดึงรายการคำถามที่ผู้ใช้ตอบไปแล้ว
