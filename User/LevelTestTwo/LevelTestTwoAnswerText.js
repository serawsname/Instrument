// File: LevelTestTwo/LevelTestTwoAnswerText.js

const express = require('express');
const router = express.Router();

module.exports = (supabase) => {
  // GET: ดึงคำถามและคำตอบสำหรับ leveltesttwo
  router.get('/leveltesttwo-answertexts', async (req, res) => {
    const { lesson_id, leveltesttwo_id } = req.query;

    if (!lesson_id || !leveltesttwo_id) {
      return res.status(400).json({ 
        status: 'error', 
        message: 'กรุณาระบุ lesson_id และ leveltesttwo_id' 
      });
    }
    ...
    try {
      ...
      res.json({ status: 'success', data: questionsWithAnswers });
    } catch (error) {
      console.error('Error fetching leveltesttwo questions:', error);
