// File: LevelTestTwo/SubmitLevelTestTwo.js

const express = require('express');
const router = express.Router();

module.exports = (supabase, authenticateToken) => {
  router.post('/submit-leveltesttwo', authenticateToken, async (req, res) => {
    const { leveltesttwo_id, lesson_id, answers } = req.body;
    const user_id = req.user.sub;
    if (!leveltesttwo_id || !lesson_id || !answers) {
      return res.status(400).json({ 
        status: 'error', 
        message: 'กรุณาระบุ leveltesttwo_id, lesson_id และ answers' 
      });
    }
    ...
    try {
      ...
      // ดึงคำถามทั้งหมดที่เกี่ยวข้องกับ leveltesttwo_id นี้
      const { data: questions, error: questionError } = await supabase
        .from('questiontext_instrument')
        .select('questiontext_id, question_text')
        .eq('leveltesttwo_id', leveltesttwo_id);
      ...
      for (const question of questions) {
        ...
        if (userAnswer && correctAnswers && correctAnswers.length > 0) {
          // ตรวจสอบกับทุกคำตอบที่ถูกต้อง
          for (const correctAnswer of correctAnswers) {
            if (typeof correctAnswer === 'string') {
              if (userAnswer.answer_text === correctAnswer) {
                isCorrect = true;
                break;
              }
            } else if (typeof correctAnswer === 'object' && correctAnswer !== null) {
              if (userAnswer.prompt && userAnswer.response) {
                if (userAnswer.prompt === correctAnswer.prompt && 
      ...
      console.error('Error submitting leveltesttwo:', error);
